...
// strips commas/spaces so "105,000" -> 105000
...
export default function CreateDocument() {
  ...
  useEffect(() => {
    apiFetch("/documents/profile/")
      .then((r) => r.json())
      .then(setProfile)
      .catch(() => {}); // redirect already handled inside apiFetch
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await apiFetch("/documents/create/", {
        method: "POST",
      ...
      const filename =
        `${safe(clientName)} ${safe(jobDescription)} ${titles[type] || type}`.trim();
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 text-gray-900">
      <form
        ...
        className="max-w-3xl mx-auto bg-white p-8 shadow border"
      >
        <div className="flex justify-between items-start border-b pb-4 mb-4">
          ...
          <div className="text-right text-sm">
            {profile.phone_1 && (
              <p>
                Tel: <strong>{profile.phone_1}</strong>
              </p>
            )}
            {profile.phone_2 && (
              <p>
                <strong>{profile.phone_2}</strong>
              </p>
            )}
        ...
        <div className="flex gap-4 mb-4">
          <div className="border p-3 w-1/2 flex flex-col gap-2">
            <input
              ...
              placeholder="M/s: Client / Company Name"
              value={clientName}
          ...
          <div className="border w-1/2">
            <p className="text-center font-bold text-lg border-b p-2">
              {titles[type] || type}
            </p>
              ...
              <tr key={idx}>
                ...
                <td className="border p-1">
                  <input
                    ...
                    onChange={(e) =>
                      updateItem(idx, "particulars", e.target.value)
                    }
                  />
                </td>
                <td className="border p-1">
